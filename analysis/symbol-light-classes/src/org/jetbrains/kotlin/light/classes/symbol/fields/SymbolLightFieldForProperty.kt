...
internal class SymbolLightFieldForProperty(
    ...
    override val kotlinOrigin: KtCallableDeclaration? = propertySymbol.psi as? KtCallableDeclaration
    ...
    private fun <T> withPropertySymbol(action: KtAnalysisSession.(KtPropertySymbol) -> T): T = analyzeForLightClasses(ktModule) {
        val symbol = if (kotlinOrigin != null) {
            when (val symbol = kotlinOrigin.getSymbol()) {
                is KtPropertySymbol -> symbol
                is KtValueParameterSymbol -> symbol.generatedPrimaryConstructorProperty
                    ?: throw KotlinExceptionWithAttachments("invalid property: $kotlinOrigin")
                        .withAttachment("fieldName.txt", fieldName)
                        .withAttachment("containingClass.txt", containingClass.text)

                else -> throw KotlinExceptionWithAttachments("illegal symbol: $symbol")
                    .withAttachment("fieldName.txt", fieldName)
                    .withAttachment("containingClass.txt", containingClass.text)
            }
        } else {
